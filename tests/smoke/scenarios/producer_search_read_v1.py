...
def test_smoke_producer_search_read(
    ...
    # Search
    search_response = producer_client_v1.search(patient_id)
    assert search_response.ok
    assert search_response.json()["total"] == len(test_pointers)

    # Search with POST
    search_response = producer_client_v1.search_post(patient_id)
    assert search_response.ok
    assert search_response.json()["total"] == len(test_pointers)

    # Read
    read_response = producer_client_v1.read(test_pointers[0].id)
